...
const lengthOfLongestSubstring = (s, startIndex = 0, endIndex = 0, longest = 0) => {
  ...
  let currentString = `${s.slice(startIndex, endIndex + 1)}`;

  // base step
  if (endIndex === s.length) return longest;

  // if all characters of current substring are unique, step endIndex by 1
  if (currentString.length === new Set(currentString.split('')).size)
  ...
  if (currentString.length !== new Set(currentString.split('')).size && startIndex < endIndex)
